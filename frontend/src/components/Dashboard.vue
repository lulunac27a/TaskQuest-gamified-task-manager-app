...
<template>
  <div class="p-6 max-w-xl mx-auto bg-slate-900 text-white rounded-xl shadow-md">
    ...
    <div class="mb-6 border-b border-slate-700 pb-4">
      ...
      <p class="text-xs text-slate-400 mt-1 text-right">{{ gameStore.user.xp }} / {{ gameStore.user.level * 100 }} XP</p>
    </div>
    ...
    <div>
      ...
      <ul class="space-y-2">
        ...
        >
          <div>
            <span :class="{ 'line-through text-slate-500': task.is_completed }">
              {{ task.title }}
            </span>
            <span class="ml-2 text-xs px-2 py-0.5 rounded bg-amber-600">
              {{ task.difficulty }}
            </span>
          </div>
          <button 
