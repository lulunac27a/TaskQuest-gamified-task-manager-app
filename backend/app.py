...
def create_app():#function to create and configure the Flask app
    ...
    app.config['SQLALCHEMY_DATABASE_URI'] = os.getenv('DATABASE_URL', 'sqlite:///taskquest.db')
    ...
    with app.app_context():
        ...
        app.register_blueprint(tasks_bp, url_prefix='/api/tasks')#prefix task routes with /api

        db.create_all()#create database tables
        # Seed a test user if none exists
        if not User.query.get(1):#check if test user with id 1 exists in database
            test_user = User(id=1, username="TestHero", xp=0, level=1, gold=100)#create test user with default values
...
if __name__ == '__main__':
    ...
    app.run(host='127.0.0.1', port=5000, debug=True)#run app on localhost with debug mode enabled
...
class User(db.Model):#user model with XP and level
    ...
    tasks = db.relationship('Task', backref='user', lazy=True)#user tasks list
